...
## Кнопка в стиле главного меню (Stitch): тёмное стекло + золотая рамка.
## primary — золотой текст и рамка ярче; иначе приглушённый тёплый текст.
func style_glass_button(btn: Button, primary: bool = false) -> void:
	var glass := Color(0.031, 0.039, 0.059, 0.4)
	var gold := Color("#d4af37")
	var bright := Color("#f2ca50")
	var bw := 2 if primary else 1
	var brd := Color(gold.r, gold.g, gold.b, 0.85 if primary else 0.55)
	btn.add_theme_stylebox_override("normal", box(glass, 13, bw, brd, 8))
	btn.add_theme_stylebox_override("hover", box(Color(gold.r, gold.g, gold.b, 0.15), 13, bw, bright, 8))
	btn.add_theme_stylebox_override("pressed", box(Color(gold.r, gold.g, gold.b, 0.24), 13, bw, bright, 8))
	btn.add_theme_stylebox_override("disabled", box(Color(0.031, 0.039, 0.059, 0.25), 13, 1, Color(gold.r, gold.g, gold.b, 0.25), 8))
	btn.add_theme_stylebox_override("focus", StyleBoxEmpty.new())
	var fc := bright if primary else Color("#d0c5af")
	...
	btn.add_theme_color_override("font_disabled_color", Color(fc.r, fc.g, fc.b, 0.5))
	if font_display:
