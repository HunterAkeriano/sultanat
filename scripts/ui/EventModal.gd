...
var _armed_at: int = 0          # до этого момента тапы игнорируются (защита от случайных)
...
func show_current() -> void:
	...
	visible = true
	# Защита от случайных нажатий: первые полсекунды после появления окна
	# тапы по вариантам не засчитываются (нельзя «проклацать» событие).
	_armed_at = Time.get_ticks_msec() + 600
	var voice := get_node_or_null("/root/Voice")
	if voice != null:
		voice.play_event_body(str(ev.id))   # озвучка тела события
...
func _populate(ev: Dictionary) -> void:
	...
	poly_sb.content_margin_bottom = 4
	...
	img.texture = _event_illustration(ev)
	img.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	img.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
	img.custom_minimum_size = Vector2(272, 352)   # крупная иллюстрация 3:4
	pv.add_child(img)
...
func _projected(key: String) -> float:
	...
	# Мультипликативный эффект на оппозицию (напр. подкуп — оппозиция вдвое ниже)
	if key == "opposition" and eff.has("opposition_mult"):
		return clampf(base * float(eff["opposition_mult"]), 0.0, 100.0)
	var d: float = GameState.event_stat_delta(key, float(eff.get(key, 0.0)))
	if key == "hazna" and _preview_ch.has("cost"):
		d -= GameState.event_choice_cost(_preview_ch)
...
var _stats_tick: float = 0.0

func _process(delta: float) -> void:
	# Обновляем статы в модалке 10 раз в секунду, а не каждый кадр — этого
	# хватает глазу, а на мобиле экономит заметное количество кадров.
	if not visible or _stat_labels.is_empty():
		return
	_stats_tick += delta
	if _stats_tick < 0.1:
		return
...
func _on_choice_up(index: int, available: bool) -> void:
	...
	# Тапы до _armed_at игнорируем — окно только что появилось (антислучайность).
	if held < 350 and available and Time.get_ticks_msec() >= _armed_at:
		_choose(index)
	...
	if ch.has("req"):
		for stat in ch.req:
			if GameState.get(stat) < ch.req[stat]:
				return false
	if ch.has("cost") and GameState.hazna < GameState.event_choice_cost(ch) \
			and not GameState.event_all_paid(GameState.active_event):
		return false
	return true
...
func _event_illustration(ev: Dictionary) -> Texture2D:
	# Явный путь image_path (напр. портрет невесты), затем индивидуальная иллюстрация
	# assets/art/events/ev_<id>.(webp|jpg|png), иначе — общая картинка из поля "image".
	var explicit: String = str(ev.get("image_path", ""))
	if explicit != "" and ResourceLoader.exists(explicit):
		return load(explicit)
	var id: String = str(ev.get("id", ""))
	for ext in [".webp", ".jpg", ".png"]:
		var p := "res://assets/art/events/ev_%s%s" % [id, ext]
		if ResourceLoader.exists(p):
			return load(p)
	return _safe_load("res://assets/art/%s.png" % ev.get("image", "event_cauldron"))
