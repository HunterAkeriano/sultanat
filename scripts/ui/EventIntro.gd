...
var _armed_at: int = 0   # до этого момента кнопка не срабатывает (защита от случайного тапа)
...
func build() -> void:
	...
	btn.pressed.connect(func():
		# Игнорируем тап, прилетевший сразу при появлении заставки.
		if Time.get_ticks_msec() >= _armed_at:
			on_open.call())
	v.add_child(btn)
...
func show_intro() -> void:
	...
	if voice != null:
		voice.play_clip("res://assets/audio/envoy_arrival.mp3")   # озвучка гонца
	visible = true
	_armed_at = Time.get_ticks_msec() + 450
