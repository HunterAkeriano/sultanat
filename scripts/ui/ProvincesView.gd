...
func _build_provisions() -> PanelContainer:
	...
	_buy_food_btn.custom_minimum_size = Vector2(0, 44)
	_buy_food_btn.pressed.connect(_on_buy_food_pressed)
...
func _build_card(p: Dictionary) -> PanelContainer:
	...
	# Статы: Souls | Лояльность + Недовольство
	...
	invest.pressed.connect(_on_invest_pressed.bind(p))
	...
	suppress.pressed.connect(_on_suppress_pressed.bind(p))
...
func _on_invest_pressed(p: Dictionary) -> void:
	var sx := get_node_or_null("/root/Sfx")
	if sx != null:
		sx.click()
	GameState.invest(p)

func _on_suppress_pressed(p: Dictionary) -> void:
	var sx := get_node_or_null("/root/Sfx")
	if sx != null:
		sx.suppress()
	GameState.suppress(p)
