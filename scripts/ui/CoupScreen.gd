...
func build() -> void:
	...
	# «Второй шанс»: ролик — и правление продолжается (1 раз за правление)
	_second_btn = Button.new()
	_second_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_second_btn.custom_minimum_size = Vector2(0, 48)
	Palette.style_glass_button(_second_btn, true)
	_second_btn.pressed.connect(_on_second_chance)
	v.add_child(_second_btn)

func _on_second_chance() -> void:
	if GameState.coup_second_used:
		return
	GameState.request_rewarded("coup_second", _grant_second_chance)

func _grant_second_chance() -> void:
	GameState.coup_second_used = true
	GameState.game_over = false
	GameState.opposition = minf(GameState.opposition, 60.0)
	GameState.notify.emit(Loc.t("over.second"), true)
	GameState.save_game()
	hide_over()

func show_over() -> void:
	_second_btn.text = "\u25B6  " + Loc.t("over.second")
	_second_btn.visible = not GameState.coup_second_used
	if _img != null and _img.texture == null:
		_img.texture = _safe_load("res://assets/art/game_over.jpg")
	var voice := get_node_or_null("/root/Voice")
	if voice != null:
		voice.play_clip("res://assets/audio/coup.mp3")   # озвучка «Вас свергли»
	move_to_front()
	visible = true
