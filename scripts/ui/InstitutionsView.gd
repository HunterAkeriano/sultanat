...
func _build_card(def: Dictionary) -> PanelContainer:
	...
	up.pressed.connect(func():
		# Тот же звук, что и при закупке зерна в Снабжении
		if GameState.can_upgrade(def):
			var sx := get_node_or_null("/root/Sfx")
			if sx != null:
				sx.buy_food()
		GameState.upgrade_institution(def))
	v.add_child(up)
...
# «58» вместо «58.0», но «63.8» для дробной скорости автосбора
func _fmt_cps(v: float) -> String:
	return str(roundi(v)) if absf(v - roundf(v)) < 0.05 else String.num(v, 1)
...
func update_view() -> void:
	for r in rows:
		...
		if str(def.get("kind", "")) == "auto_click":
			if lvl <= 0:
				# Доступ ещё не куплен: показываем, что даст 1-й уровень
				r.eff.text = GameData.loc(def, "effect") % _fmt_cps(float(def.base_cps))
				r.up.text = "%s · %s \u269C" % [Loc.t("inst.unlock"), Palette.fmt(cost)]
			else:
				r.eff.text = GameData.loc(def, "effect") % _fmt_cps(GameState.auto_clicks_per_sec())
